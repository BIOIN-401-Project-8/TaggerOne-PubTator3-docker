...
public class AnnotatedSegment extends Segment {
	...
	protected double entityScore;

	public AnnotatedSegment(TextInstance text, int startChar, int endChar, List<Token> tokens, Set<Entity> entities, double entityScore) {
		...
		setEntities(entities, entityScore);
	}
	...
	public void setEntities(Set<Entity> entities, double entityScore) {
		if (entities == null) {
			this.entities = null;
		} else {
	...
	public AnnotatedSegment copy() {
		AnnotatedSegment copy = new AnnotatedSegment(sourceText, startChar, endChar, tokens, entities, entityScore);
		copy.mentionName = mentionName;
