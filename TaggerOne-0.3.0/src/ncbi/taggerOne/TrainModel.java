...
import ncbi.taggerOne.processing.textInstance.UnknownNormalizationIterationWrapper;
...
public class TrainModel {
	...
	public static void main(String[] args) {
		...
		WeightedVectorSpaceExtractor mentionVectorSpaceExtractor = new WeightedVectorSpaceExtractor(mentionVectorSpace);
		...
		for (String entityType : entityTypeSet) {
			...
			WeightedVectorSpaceExtractor nameVectorSpaceExtractor = new WeightedVectorSpaceExtractor(nameVectorSpace);
			MentionNameProcessor extractorPipeline = new MentionNameProcessingPipeline(nameApplicator, entityNameTokenizer, tokenApplicator, mentionVectorSpaceExtractor, nameVectorSpaceExtractor);
			...
			Vector<String> nameVectorSpaceWeights = nameVectorSpaceExtractor.getWeights();
			Int2IntOpenHashMap frequencies = nameVectorSpaceExtractor.getFrequencies();
		...
		List<FeatureProcessor> featureProcessors = getFeatureProcessors(maximumSegmentLength);
		...
		Vector<String> mentionVectorSpaceWeights = mentionVectorSpaceExtractor.getWeights();
		Int2IntOpenHashMap mentionTokenFrequencies = mentionVectorSpaceExtractor.getFrequencies();
		for (int i = 0; i < mentionVectorSpace.size(); i++) {
			String element = mentionVectorSpace.getElement(i);
			Pattern pattern = Pattern.compile("\\s");
			Matcher matcher = pattern.matcher(element);
			if (matcher.find()) {
				logger.error("Name vector space element \"" + element + "\" contains whitespace");
			}
			double weight = mentionVectorSpaceWeights.get(i);
			if (weight > 0.0) {
				logger.trace("Mention element #" + i + ": \"" + element + "\" has weight " + weight + " from frequency " + mentionTokenFrequencies.get(i));
			} else {
				logger.error("Mention element #" + i + ": \"" + element + "\" has non-positive weight " + weight + " from frequency " + mentionTokenFrequencies.get(i));
			}
		}
		...
		start = System.currentTimeMillis();
		// FIXME simplify: have to convert to vectors to calculate the token
		// weights, then reconvert to weighted vectors
		for (String entityType : entityTypeSet) {
		...
		MentionNameProcessingPipeline mentionConverter = new MentionNameProcessingPipeline(nameApplicator, entityNameTokenizer, tokenApplicator,
				new TokenListToWeightedVectorConverter(SparseVector.factory, mentionVectorSpace, mentionVectorSpaceWeights, false, false));
		...
		UnknownNormalizationIterationWrapper unknownNormalizationIterationWrapper = new UnknownNormalizationIterationWrapper(trainingIteration, lexicon, normalizationTrainingPredictionModels);
		int trainingPipelineReportingIncrement = options.valueOf(useSentenceBreaker) ? 100 : 10;
		TextInstanceProcessingPipeline trainingPipeline = new TextInstanceProcessingPipeline(new ProgressReporter("TrainingPipeline", trainingPipelineReportingIncrement), unknownNormalizationIterationWrapper,
				new InstanceElementClearer(EnumSet.of(InstanceElement.PredictedStates, InstanceElement.PredictedAnnotations)));
