...
public class QuadraticProgram {
	...
	public QuadraticProgram convertQP(boolean[] retainVariables) {
		QuadraticProgram convertedQP = new QuadraticProgram(lexicon, recognitionFeatureSet, entityClassStates, mentionIndexToNameIndex, normalizationTypeToVars.keySet(), regularization, maxStepSize, solverTimeout);
		...
		logger.debug("\tMinimal QP has " + convertedQP.varCount + " variables out of " + varCount);
	...
	public boolean solve(Matrix<String, String> featureWeightUpdates, TObjectDoubleMap<String> cosineSimUpdates, Map<String, Matrix<String, String>> normalizationTypeToWeightUpdates) {
		...
		logger.debug("\tSolution state = " + result.getState().toString());
		...
		logger.debug("\tSize of full update = " + Math.sqrt(sqrSum));
	...
	private static class SolverThread extends Thread {
		...
		public void run() {
			...
			result = solver.solve();
			elapsed += System.currentTimeMillis();
			logger.debug("\tQP solution took " + elapsed + " ms");
		}
	}
	...
	public void addNonNegativeCosineSimNormalizationConstraints(Map<String, NormalizationModelUpdater> normalizationUpdaterModels) {
		for (String entityType : cosineSimVars.keySet()) {
			...
			logger.debug("\tAdding non-negative cosine similarity constraint for type \"" + entityType + "\"");
			addConstraint(diagonal);
		}
	}

	private double copyRecognitionResult(Result result, double stepSize, Matrix<String, String> featureWeightUpdates) {
		double sqrSum = 0.0;
		long[] keys = recognitionVars.keys();
		for (int i = 0; i < keys.length; i++) {
			long joint = keys[i];
			...
			sqrSum += value * value;
			featureWeightUpdates.set(state, index, value);
		}
		logger.debug("\tSize of recognition update = " + Math.sqrt(sqrSum));
		return sqrSum;
	}
	...
	private double copyCosineSimResult(Result result, double stepSize, TObjectDoubleMap<String> cosineSimUpdates) {
		...
		for (String entityType : cosineSimVars.keySet()) {
			...
			logger.debug("\tSize of cosine sim update for type " + entityType + " = " + Math.sqrt(sqrSum));
			totalSqrSum += sqrSum;
		}
		return totalSqrSum;
	}
	...
	private double copyNormalizationResult(Result result, double stepSize, Map<String, Matrix<String, String>> normalizationTypeToWeightUpdates) {
		...
		for (String entityType : normalizationTypeToVars.keySet()) {
			...
			logger.debug("\tSize of normalization update for type " + entityType + " = " + Math.sqrt(sqrSum));
			totalSqrSum += sqrSum;
		}
		return totalSqrSum;
	}
	...
	private void checkSlackResult(Result result) {
		...
		logger.debug("\tSize of slack loading = " + sum);
	}
