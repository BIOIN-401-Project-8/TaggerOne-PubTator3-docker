...
public class MIRAUpdate implements OnlineOptimizer {
	...
	public MIRAUpdate(Lexicon lexicon, Dictionary<String> mentionVectorSpace, Dictionary<String> recognitionFeatureSet, RecognitionModelPredictor recognitionModelPredictor, RecognitionModelUpdater recognitionModelUpdater,
			Map<String, NormalizationModelPredictor> normalizationPredictionModels, Map<String, NormalizationModelUpdater> normalizationUpdaterModels, double regularization, double maxStepSize, long solverTimeout, int topNLabelings,
			int topNNormalization, boolean enforceNonNegativeDiagonal) {
	...
	@Override
	public void update(List<AnnotatedSegment> targetStateSequence, List<AnnotatedSegment> predictedStateSequence, double margin) {
		Profiler.start("MIRAUpdate.update()");
		Profiler.start("MIRAUpdate.update()@setup");

		logger.debug("Updating sequence");
		logger.debug("\ttarget= " + AnnotatedSegment.visualizeStates(targetStateSequence));
		logger.debug("\tprediction= " + AnnotatedSegment.visualizeStates(predictedStateSequence));
		logger.debug("\textra margin= " + margin);

		QuadraticProgram qp = new QuadraticProgram(lexicon, recognitionFeatureSet, recognitionModelPredictor.getEntityClassStates(), mentionIndexToNameIndex, normalizationPredictionModels.keySet(), regularization,
				maxStepSize, solverTimeout);
		...
		logger.debug("\tNER loss = " + loss);
		baseConstraint.addBValue(2.0 * loss - margin);
		...
		for (int i = 0; i < constraints.size(); i++) {
			logger.debug("\t" + i + ": loss = " + constraints.get(i).getB());
		}
		...
		logger.debug("After adding incorrect normalizations:");
		for (int i = 0; i < constraints.size(); i++) {
			logger.debug("\t" + i + ": loss = " + constraints.get(i).getB());
		}
		...
		if (topNNormalization > 0) {
			...
			logger.debug("After adding normalization only:");
			for (int i = 0; i < constraints.size(); i++) {
				logger.debug("\t" + i + ": loss = " + constraints.get(i).getB());
			}
		}
		...
		qp = qp.getMinimalQP();
		logger.debug("\tNumber of constraints: " + qp.getConstraintCount());
		logger.debug("\tNumber of variables: " + qp.getVariableCount());
		...
		Matrix<String, String> featureWeightUpdates = new SparseMatrix<String, String>(entityClassStates, featureSet);
		...
		for (String entityType : normalizationUpdaterModels.keySet()) {
			Index index = lexicon.getIndex(entityType);
			Matrix<String, String> normalizationWeightUpdate = new SparseMatrix<String, String>(mentionVectorSpace, index.getNameVectorSpace());
			...
			if (!entityType.equals(T1Constants.NONENTITY_STATE)) {
				...
				if (mentionVector != null) {
					...
					if (predictedSegment == null) {
						...
						if (!(normalizationModelPredictor instanceof NullNormalizationModel)) {
							...
							for (Entity entity : annotatedSegment.getEntities()) {
								...
								logger.debug("\tAdding NER FN normalization: mention = " + mentionVector.visualize() + ", type = " + entityType + ", name = " + correctNameVector.visualize() + ", loss = " + loss);
					...
					if (annotatedSegment == null) {
						...
						if (!(normalizationModelPredictor instanceof NullNormalizationModel)) {
							...
							for (int rank = 0; rank < bestEntities.size(); rank++) {
								...
								for (QPConstraint constraint2 : constraints) {
									QPConstraint constraintCopy = constraint2.copy();
									logger.debug("\tAdding NER FP normalization: mention = " + mentionVector.visualize() + ", type = " + entityType + ", name = " + nameVector.visualize() + ", loss = " + loss);
								...
								if (correctNameVector != null) {
									...
									for (int rank = 0; rank < bestEntities2.size() && added < topNLabelings; rank++) {
										...
										if (score > correctNameVectorScore) {
											...
											logger.debug("\tAdding incorrect normalization: mention = " + mentionVector.visualize() + ", type = " + ", correct name = " + correctNameVector.visualize() + ", incorrect name = "
													+ incorrectNameVector.visualize() + ", loss = " + loss);
											for (QPConstraint constraint2 : constraints) {
												QPConstraint constraintCopy = constraint2.copy();
												constraintCopy.addNormalization(entityType, mentionVector, correctNameVector, true);
							...
							for (int rank = 0; rank < bestEntities.size() && added < topNNormalization; rank++) {
								...
								if (bestEntities.getValue(rank) >= correctNameVectorScore) {
									...
									if (!segment.getEntities().contains(entityFound)) {
										...
										logger.debug("\tAdding normalization only constraint: mention = " + mentionVector.visualize() + ", type = " + ", correct name = " + correctNameVector.visualize() + ", incorrect name = "
												+ nameVector.visualize());
