...
import ncbi.taggerOne.processing.features.token.BrownClustersFeatureProcessor;
...
import ncbi.taggerOne.processing.features.token.WordVectorClusterFeatureProcessor;
...
import ncbi.taggerOne.processing.stoppingCriteria.FixedModelNameFormatter;
...
import ncbi.taggerOne.util.tokenization.SimpleTokenizer;
...
public class EzTrain {

	private static final Logger logger = LoggerFactory.getLogger(EzTrain.class);
	...
	public static void main(String[] args) {
		...
		OptionSpec<Double> holdoutPercentage = parser.accepts("holdoutPercentage").withRequiredArg().ofType(Double.class).required();
		...
		logger.info("Creating lexicon loaders");
		...
		try {
			for (String lexiconConfig : options.valuesOf(lexiconConfigs)) {
				...
				loaders.add(loader);
				lexiconNamespaceSet.addAll(loader.getNamespaceSet());
				lexiconProvided = true;
			}
		} catch (ReflectiveOperationException e) {
			throw new RuntimeException(e);
		}
		List<Dataset> datasets = getDatasets(options.valuesOf(trainingDatasetConfigs));
		Set<String> entityTypeSet = new HashSet<String>();
		for (Dataset dataset : datasets) {
			entityTypeSet.addAll(dataset.getEntityTypes());
		}
		...
		boolean entityAnnotationsProvided = false;
		for (Dataset dataset : datasets) {
			dataset.setLexicon(lexicon);
			entityAnnotationsProvided = entityAnnotationsProvided || dataset.containsEntityAnnotations();
			trainingInstances.addAll(dataset.getInstances());
		}
		...
		logger.info("Separated " + holdoutInstances.size() + " holdout instances");
		...
		LexicalFeatureProcessor lexicalFeatureProcessor = null;
		if (true) { // TODO Configure
			...
			StringProcessingPipeline tokenProcessor = new StringProcessingPipeline(new LowerCaseStringProcessor(), stemmer);
			lexicalFeatureProcessor = new LexicalFeatureProcessor("LEX", textInstanceTokenizer, segmentProcessor, tokenProcessor);
		}
		List<FeatureProcessor> featureProcessors = getFeatureProcessors(maxSegmentLength, lexicalFeatureProcessor);
		...
		for (String entityType : entityTypeSet) {
			...
			TokenWeightCalculator tokenWeightCalculator = new TokenWeightCalculator(nameVectorSpace, lexicon.getEntities(entityType));
		...
		for (String entityType : entityTypeSet) {
			Index entityIndex = lexicon.getIndex(entityType);
			boolean isNEROnly = !lexiconProvided || !entityAnnotationsProvided;
			// 1. If concept annotation is in corpus and lexicon is provided -> normalization
			// 2. If concept annotation is in corpus but no lexicon is provided -> NER
			// 3. If no concept annotation but lexicon provided -> NER + dictionary match
			// 4. If no concept annotation and no lexicon is provided -> NER
			if (isNEROnly) {
				NullNormalizationModel entityModel = new NullNormalizationModel(entityIndex);
				entityModels.put(entityType, entityModel);
				normalizationTrainingPredictionModels.put(entityType, entityModel);
		...
		EvaluationProcessorStoppingCriteria stoppingCriteria = new EvaluationProcessorStoppingCriteria(options.valueOf(maxTrainingIterations), options.valueOf(iterationsPastLastImprovement), trainingProgressTracker, holdoutInstances,
				preEvaluationPipeline, postEvaluationPipeline, annotationPipeline, new FixedModelNameFormatter(options.valueOf(modelOutputFilename)), evaluationProcessors, additionalProcessors);

		logger.info("Elapsed = " + (System.currentTimeMillis() - start));

		logger.info("Training");
		start = System.currentTimeMillis();
		OnlineOptimizer optimizer = new MIRAUpdate(lexicon, mentionVectorSpace, featureSet, trainingRecognitionPredictor, recognitionModel, normalizationTrainingPredictionModels, normalizationUpdaterModels, options.valueOf(regularization),
	...
	private static List<Dataset> getDatasets(List<String> datasetConfigs) {
	...
	private static List<FeatureProcessor> getFeatureProcessors(int maxSegmentLength, LexicalFeatureProcessor lexicalFeatureProcessor) {
		...
		processors.add(new StartEndTokenFeatureProcessor("BWC", new PatternProcessor(PatternProcessor.BRIEF_CHARACTER_CLASS_PATTERNS, PatternProcessor.CHARACTER_CLASS_REPLACEMENTS)));
		...
		if (lexicalFeatureProcessor != null) {
			processors.add(lexicalFeatureProcessor);
		}

		// TODO Make BRWN and WVC configurable
		int[] prefixLengths = { 4, 6, 10, 20 };
		// int[] prefixLengths = { 4, 6, 10 };
		processors.add(new BrownClustersFeatureProcessor("BRWN", "data/BrownHierarchicalWordClusters/merged-abs-simple-tokenized-c1000-p1.txt.gz", 0, null, prefixLengths));
		WordVectorClusterFeatureProcessor wordVectorClusterFeatureProcessor3 = new WordVectorClusterFeatureProcessor("WVC3", new SimpleTokenizer());
		wordVectorClusterFeatureProcessor3.loadFromFile("data/WordVectorClusters/whitespace-tokenized-300.classes.sorted.txt");
		WordVectorClusterFeatureProcessor wordVectorClusterFeatureProcessor5 = new WordVectorClusterFeatureProcessor("WVC5", new SimpleTokenizer());
		wordVectorClusterFeatureProcessor5.loadFromFile("data/WordVectorClusters/whitespace-tokenized-500.classes.sorted.txt");
		WordVectorClusterFeatureProcessor wordVectorClusterFeatureProcessor10 = new WordVectorClusterFeatureProcessor("WVC10", new SimpleTokenizer());
		wordVectorClusterFeatureProcessor10.loadFromFile("data/WordVectorClusters/whitespace-tokenized-1000.classes.sorted.txt");
