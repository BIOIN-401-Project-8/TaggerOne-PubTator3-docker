...
public class NLMChemBioCDataset implements Dataset {

	private static final Logger logger = LoggerFactory.getLogger(NLMChemBioCDataset.class);
	...
	private void loadInstances(String filename, List<TextInstance> instances) {
		...
		try {
			InputStreamReader reader = new InputStreamReader(new FileInputStream(filename), T1Constants.UTF8_FORMAT);
			connector.startRead(reader);
	...
	private Set<Entity> getEntities(BioCAnnotation annotation) {
		...
		if (!usageStr.equals(Dataset.Usage.IDENTIFY.name())) {
			...
			entities = getEntities(identifierStr, entityType);
		}
		return entities;
	}
