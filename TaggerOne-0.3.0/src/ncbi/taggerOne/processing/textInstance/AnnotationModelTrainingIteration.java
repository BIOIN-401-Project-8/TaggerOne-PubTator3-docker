...
public class AnnotationModelTrainingIteration extends TextInstanceProcessor {
	...
	private static final Logger logger = LoggerFactory.getLogger(AnnotationModelTrainingIteration.class);
	...
	public void process(TextInstance input) {
		...
		logger.debug(input.getInstanceId() + ": Getting target state sequence; elapsed=" + (System.currentTimeMillis() - start));
		List<AnnotatedSegment> targetStateSequence = input.getTargetStateSequence();
		logger.debug(input.getInstanceId() + ": Getting predicted state sequences; elapsed=" + (System.currentTimeMillis() - start));
		...
		if (!targetStateSequence.equals(predictedStateSequence)) {
			...
			logger.debug(input.getInstanceId() + ": Updating; target score = " + String.format(T1Constants.SCORING_FORMAT, targetScore) + ", predicted score = " + String.format(T1Constants.SCORING_FORMAT, predictedScore) + ", difference = "
					+ String.format(T1Constants.SCORING_FORMAT, predictedScore - targetScore));
			...
			logger.debug("Updating with extra margin = " + margin + ", iteration = " + callback.getIteration() + ", updates = " + callback.getUpdates());
			optimizer.update(targetStateSequence, predictedStateSequence, margin);
			...
			logger.debug(input.getInstanceId() + ": After update; target score = " + String.format(T1Constants.SCORING_FORMAT, targetScore) + ", predicted score = " + String.format(T1Constants.SCORING_FORMAT, predictedScore)
					+ ", difference = " + String.format(T1Constants.SCORING_FORMAT, predictedScore - targetScore));
			logger.debug(input.getInstanceId() + ": Updates complete; elapsed=" + (System.currentTimeMillis() - start));
			if (!hasUpdates) {
				hasUpdates = Math.abs(targetScore) > T1Constants.EPSILON && Math.abs(predictedScore) > T1Constants.EPSILON;
			}
			if (callback != null) {
