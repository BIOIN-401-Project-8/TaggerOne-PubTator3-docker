...
public class SentenceBreaker implements Serializable {
	...
	public List<TextInstance> breakSentences(List<TextInstance> instances) {
		...
		logger.debug("Document instances = " + instances.size() + " sentence instances = " + sentenceInstances.size());
	...
	private static TextInstance getSentenceInstance(TextInstance instance, int offset, int start, int counter, String sentenceText) {
		...
		TextInstance sentenceInstance = new TextInstance(instance, instanceId, sourceId, sentenceText, offset + start);
		...
		if (originalTargetAnnotation != null) {
			...
			for (AnnotatedSegment originalAnnotation : originalTargetAnnotation) {
				AnnotatedSegment newAnnotation = getSentenceAnnotation(originalAnnotation, sentenceInstance, start);
	...
	private static AnnotatedSegment getSentenceAnnotation(AnnotatedSegment annotation, TextInstance sentenceInstance, int offset) {
		...
		if (newStartChar >= 0 && newEndChar <= length) {
			...
			return new AnnotatedSegment(sentenceInstance, newStartChar, newEndChar, null, annotation.getEntities(), annotation.getEntityScore());
		}
		// Overlap
