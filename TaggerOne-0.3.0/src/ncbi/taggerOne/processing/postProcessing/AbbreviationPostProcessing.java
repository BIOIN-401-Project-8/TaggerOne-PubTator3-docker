...
public class AbbreviationPostProcessing extends TextInstanceProcessor {
	...
	public AbbreviationPostProcessing(AbbreviationResolver abbreviationResolver, int changeThreshold, int addThreshold,
			boolean dropIfNoExpandedPrediction) {
	...
	public void processAll(List<TextInstance> input) {
		...
		TObjectDoubleHashMap<String> mentionTextToEntityScore = new TObjectDoubleHashMap<String>();
		for (TextInstance instance : input) {
			...
			for (AnnotatedSegment segment : predictedAnnotation) {
				...
				if (segment.getEntityScore() > mentionTextToEntityScore.get(hashKey)) {
					mentionTextToEntityScore.put(hashKey, segment.getEntityScore());
				}
			}
		}
		...
		for (TextInstance instance : input) {
			List<AnnotatedSegment> predictedAnnotationsCopy = new ArrayList<AnnotatedSegment>(
					instance.getPredictedAnnotations().getObject(0));
			...
			for (Segment segment : instance.getSegments()) {
				...
				if (!mentionText.contains("(") && !mentionText.contains(")") && !expandedText.equals(mentionText)) {
					...
					double expandedEntityScore = mentionTextToEntityScore.get(hashKey);
					...
					// Determine the target annotation for this segment (so the logging will reflect
					// how well this
					...
					String logText = instance.getSourceId() + "\t" + mentionText + "\t" + expandedText + "\t"
							+ expandedTypeCount + "\t" + predictedType + "\t" + overlapType + "\t" + expandedType + "\t"
							+ targetType + "\t" + expandedEntitySet + "\t" + expandedEntityScore;
					...
					if (predictedType == null) {
						...
						if (expandedType != null) {
							...
							if (expandedTypeCount >= addThreshold && overlapSegment == null) {
								...
								newAnnotation.setEntities(new HashSet<Entity>(expandedEntitySet), expandedEntityScore);
						...
						if (expandedType == null) {
							...
							// Check if we should change the current prediction to the expanded text
							// prediction
