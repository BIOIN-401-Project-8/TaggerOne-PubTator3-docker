...
public class InstanceLevelEvaluationProcessor extends EvaluationProcessor {
	...
	@Override
	public InstanceLevelEvaluationProcessor copy() {
		return new InstanceLevelEvaluationProcessor(scoreDetailPrefix);
	}
	...
	public Object2DoubleMap<String> getScores() {
		...
		Object2DoubleMap<String> overallScores = score.getScores();
		for (String s : overallScores.keySet()) {
			String s2 = String.format("INSTANCE\t%s\t%s", T1Constants.OVERALL_EVALUATION, s);
		...
		for (String entityType : entityTypes) {
			...
			Object2DoubleMap<String> scoresForType = score.getScores();
			for (String s : scoresForType.keySet()) {
				String s2 = String.format("INSTANCE\t%s\t%s", entityType, s);
